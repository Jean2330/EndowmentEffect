...
# =============================================================================
# PART 15: SUMMARY AND INTERPRETATION
# =============================================================================

cat("\n=== SUMMARY OF SES EFFECTS ===\n\n")

# Calculate endowment effect by SES
dif_low <- stats_11_ses %>% 
  filter(ses_level == "Low SES") %>% 
  pull(Mean) %>% 
  {.[1] - .[2]}

dif_med <- stats_11_ses %>% 
  filter(ses_level == "Medium SES") %>% 
  pull(Mean) %>% 
  {.[1] - .[2]}

dif_high <- stats_11_ses %>% 
  filter(ses_level == "High SES") %>% 
  pull(Mean) %>% 
  {.[1] - .[2]}

cat("EXPERIMENT 1.1 - Endowment Effect by SES:\n")
cat("  Low SES:    Difference =", round(dif_low, 1), "pesos\n")
cat("  Medium SES: Difference =", round(dif_med, 1), "pesos\n")
cat("  High SES:   Difference =", round(dif_high, 1), "pesos\n\n")

cat("EXPERIMENT 1.1 - SES Coefficient Interpretation:\n")
cat("  SES Index coefficient (M3):", round(coef(probit_11_m3)["ses_index_c"], 4), "\n")
if(summary(probit_11_m3)$coefficients["ses_index_c", "Pr(>|z|)"] < 0.05) {
  cat("  Result: SES has SIGNIFICANT effect on valuation\n")
} else {
  cat("  Result: SES effect not significant\n")
}
cat("\n")

cat("EXPERIMENT 1.2 - SES Coefficient Interpretation:\n")
cat("  SES Index coefficient (M3):", round(coef(probit_12_m3)["ses_index_c"], 4), "\n")
if(summary(probit_12_m3)$coefficients["ses_index_c", "Pr(>|z|)"] < 0.05) {
  cat("  Result: SES has SIGNIFICANT effect on valuation\n")
} else {
  cat("  Result: SES effect not significant\n")
}
...
cat("EXPERIMENT 2 - SES Coefficient Interpretation:\n")
cat("  SES Index coefficient (M3):", round(coef(probit_2_m3)["ses_index_c"], 4), "\n")
if(summary(probit_2_m3)$coefficients["ses_index_c", "Pr(>|z|)"] < 0.05) {
  cat("  Result: SES has SIGNIFICANT effect on broker decisions\n")
} else {
  cat("  Result: SES effect not significant in broker context\n")
}
cat("\n")
...
cat("  - SES index validation\n")
cat("  - Descriptive statistics by SES level\n")
cat("  - SES × Role interaction plots (3 experiments)\n")
cat("  - Statistical tests by SES\n")
cat("  - Probit regression models with SES controls (15 models total)\n")
cat("  - Marginal effects\n")
cat("  - Predicted probability curves by SES\n\n")
...
if (save_outputs) {
  ...
  dir.create("outputs/ses_analysis/tables", recursive = TRUE, showWarnings = FALSE)
  dir.create("outputs/ses_analysis/figures", recursive = TRUE, showWarnings = FALSE)
  
  cat("\nSaving outputs...\n")
  
  # Save tables
  write.csv(table_ses, "outputs/ses_analysis/tables/ses_distribution.csv", row.names = FALSE)
  write.csv(components_ses, "outputs/ses_analysis/tables/ses_components.csv", row.names = FALSE)
  write.csv(stats_11_ses, "outputs/ses_analysis/tables/stats_exp11_ses.csv", row.names = FALSE)
  write.csv(stats_12_ses, "outputs/ses_analysis/tables/stats_exp12_ses.csv", row.names = FALSE)
  write.csv(stats_2_ses, "outputs/ses_analysis/tables/stats_exp2_ses.csv", row.names = FALSE)
  write.csv(test_11_ses, "outputs/ses_analysis/tables/test_exp11_ses.csv", row.names = FALSE)
  write.csv(test_12_ses, "outputs/ses_analysis/tables/test_exp12_ses.csv", row.names = FALSE)
  
  # Save figures
  ggsave("outputs/ses_analysis/figures/ses_distribution.png", 
         plot = p_ses_dist, width = 10, height = 6, dpi = 300)
  
  ggsave("outputs/ses_analysis/figures/ses_components.png", 
         plot = p_components, width = 10, height = 6, dpi = 300)
  
  ggsave("outputs/ses_analysis/figures/exp11_ses_interaction.png", 
         plot = p1_ses_interact, width = 10, height = 6, dpi = 300)
  
  ggsave("outputs/ses_analysis/figures/exp12_ses_interaction.png", 
         plot = p2_ses_interact, width = 10, height = 6, dpi = 300)
  
  ggsave("outputs/ses_analysis/figures/exp2_ses_interaction.png", 
         plot = p3_ses_interact, width = 10, height = 6, dpi = 300)
  
  ggsave("outputs/ses_analysis/figures/exp11_boxplot_ses.png", 
         plot = p_box_11, width = 10, height = 6, dpi = 300)
  
  ggsave("outputs/ses_analysis/figures/exp12_boxplot_ses.png", 
         plot = p_box_12, width = 10, height = 6, dpi = 300)
  
  ggsave("outputs/ses_analysis/figures/exp11_pred_prob_ses.png", 
         plot = p1_pred_ses, width = 10, height = 6, dpi = 300)
  
  ggsave("outputs/ses_analysis/figures/exp12_pred_prob_ses.png", 
         plot = p2_pred_ses, width = 10, height = 6, dpi = 300)
  
  ggsave("outputs/ses_analysis/figures/exp2_pred_prob_ses.png", 
         plot = p3_pred_ses, width = 10, height = 6, dpi = 300)
  
  cat("Outputs saved successfully to outputs/ses_analysis/\n")
  cat("  Tables: 7 files\n")
  cat("  Figures: 10 files\n")
}
